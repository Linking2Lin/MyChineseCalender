...
缓存写入或清理失败时，进程内仍保留当天最新有效数据，下次普通加载尝试补写与清理。不同日期按分组独立协调请求，明日预取不会阻塞今天的加载。诗词 Token 读写故障不阻断可用的网络请求，认证失效最多重试一次。
...
维护入口、核心流程与按修改需求选择的测试见 [代码维护导航](docs/MAINTENANCE.md)。源码中的中文注释标明函数入参、返回结果或副作用，并解释日期身份、缓存错误、组件生命周期、图片提交顺序及持久化兼容约定。

本轮处理情况与验证边界见 [2026-09-13 审查及修复报告](docs/REVIEW_AND_FIXES_2026-09-13.md)。此前记录见 [2026-09-06 审查报告](docs/CODE_REVIEW_2026-09-06.md) 和 [修复记录](docs/FIXES_2026-09-06.md)。

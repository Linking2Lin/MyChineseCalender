...
小组件的手动点击与后台同步均优先复用当天有效缓存：同一日期成功获取后不重复请求接口，没有有效缓存时才联网，失败后可以继续重试；已预取的当日缓存同样直接复用。重建会话时先读取当天缓存，已有内容在刷新期间继续展示。网络失败时，有当天有效缓存就继续展示，没有则使用空态。空态在所有尺寸下均复用中等布局的样式和头像，仅将第一行固定为“常能遣其欲而心自静，”，第二行固定为“澄其心而神自清。”。此规则仅用于小组件农历，App 页面黄历和诗词的刷新行为保持原样。
...
首次维护建议沿页面或小组件的一条调用链阅读，再参考导航中的状态字段说明、测试时序说明和构建资源索引。测试函数已标注触发条件与预期结果，生成文件及资源文件的职责集中在导航中说明。
